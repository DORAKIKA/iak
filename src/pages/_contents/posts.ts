...
// 获取相关文章
export function getRelatedPosts(target: CollectionEntry<'posts'>, count: number){
  // tags, category
  ...
  posts.forEach(post => {
    ...
    // category 权重
    if(post.data.category === target.data.category && target.data.category !== undefined){
      value[1] = 1;
    }
...
export function getPrevAndNext(target: CollectionEntry<'posts'>){
  let nextPost: CollectionEntry<'posts'> | undefined, prevPost: CollectionEntry<'posts'> | undefined;
  const posts = getPosts();
  posts.forEach(post => {
