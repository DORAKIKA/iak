...
<template>
  <teleport to="body">
    <transition name="pop">
      <div
        v-show="show"
        class="the-contextmenu"
        :style="{ left: position.x + 'px', top: position.y + 'px' }"
        ref="contextmenuRef"
      >
        <div
          class="the-contextmenu__group"
          :class="group.type"
          v-for="group in config"
          :style="group.color && `--main-color-meta: ${group.color}`"
        >
          <button
            class="the-contextmenu__item"
            v-for="item in group.children"
            :key="group.type + item.icon"
            @click="item.action(group.target)"
          >
            <div class="the-contextmenu__item__icon" v-if="item.icon">
              <i :class="item.icon"></i>
            </div>
            <div class="the-contextmenu__item__text" v-if="item.text">
              {{ item.text }}
            </div>
            <div
              class="the-contextmenu__item__tag"
              v-if="group.name || item.tag"
            >
              {{ group.name || item.tag }}
            </div>
          </button>
...
.pop-enter-active,
.pop-leave-active {
  transition: 0.2s cubic-bezier(0.12, 0.72, 0.41, 1.2);
}
</style>
