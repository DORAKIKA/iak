...
const tags = Array.from(tags_map)
  .sort((a: any, b: any) => {
    return b[1] - a[1];
  })
  .map((item: any) => {
...
<template>
  <aside class="panel-site">
    <section class="panel-card panel-tags">
      ...
      <div class="tags">
        <a :href="`/tag/${tag}`" v-for="tag in tags" :key="tag" class="tag">{{
          tag
        }}</a>
      </div>
    </section>
    ...
    <section class="panel-card panel-mark">
      ...
      <div class="mark-item">
        ...
        <div class="mark-meta">
          ...
          <div class="mark-item__time">
            {{ formatDateTime(new Date(mark.time)) }}
          </div>
...
.mark-item {
  ...
  background-image: linear-gradient(
    to right,
    rgba(var(--main-color-meta), 0.15),
    rgba(var(--main-color-meta), 0.2) 80%
  );
  user-select: none;
}
.mark-item .mark-meta {
