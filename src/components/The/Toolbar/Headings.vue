...
const headings = props.headings.filter(
  (h: any) => h.depth <= headingsConfig.depth
);
...
watch(headingsShow, () => {
  if (headingsShow.value) {
    setTimeout(() => {
      let firstHeadingItem: HTMLElement | null = document.querySelector(
        ".the-sidebar .toolbar-headings-item"
      );
    ...
    setTimeout(() => {
      let trigger: HTMLElement | null =
        document.querySelector("#headings-trigger");
      if (trigger) trigger.focus();
    }, 300);
  }
});

registryFunction("toggleHeadings", toggleHeadings);
</script>

<template>
  <button
    id="headings-trigger"
    class="toolbar-headings-trigger"
    :class="className"
    @click="toggleHeadings(true)"
  >
    <svg
      class="icon"
    ...
    >
      <path
        d="M120.832 35.84h173.056v403.456h436.224V35.84h173.056v952.32H730.112v-399.36H293.888v399.36H120.832v-952.32z"
        p-id="4656"
      ></path>
    </svg>
  </button>
